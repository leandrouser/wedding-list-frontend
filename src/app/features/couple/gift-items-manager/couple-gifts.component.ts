...
@Component({
  ...
  templateUrl: './couple-gifts.component.html',
  styleUrls: ['./couple-gifts.component.css']
})
export class CoupleGiftsComponent implements OnInit {
  ...
  ngOnInit(): void {
    const listId = this.authService.currentGiftListId;
    if (!listId) {
    ...
    this.giftListService.getGiftListById(listId).subscribe({
      next: list => {
        this.list.set(list);
